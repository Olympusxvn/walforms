...
Builder / Form filler / Dashboard / Verifier (static HTML/CSS/JS)
          │
          ├── walrus.js  ── PUT/GET ──► Walrus mainnet publisher / aggregator
          │                             (fallback chain + curl fallback UI)
          │
          ├── sui.js  ──── TX ────────► Sui Move contract (walforms.move)
          │                             create_form / record_submission / seal_form
          │
          └── crypto.js  (SHA-256, Merkle root, bytesToHex)

On-chain: WalForm object stores creator, title, submissionCount,
          blobIds[], submissionHashes[], final_manifest_root (sealed).

Off-chain (local browser only): priority flags, notes — IndexedDB, not on-chain.
```
...
- **Seal encryption (Seal V2):** The integrity manifest uses on-chain Merkle root verification only. Encryption of individual submissions via Seal is a V2 feature — not implemented in this release.
- **CORS on browser uploads:** Public Walrus publishers may reject browser PUT requests from some origins. The UI shows a curl fallback command when this happens — do not paste fake success.
...
**In WalForms's case the answer is no — and that's the point. Most feedback tools fail this test.**
